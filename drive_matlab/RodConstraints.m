...
switch desFunc
    case 'G' % position level :rod has correct length
        ...
        g = G;
    case 'GDOT' % Velocity level:rod endpoints aren't moving in a way that changes the length
        numRods = size(omega.R,1);  
        GDOT = zeros(numRods,1,class(X.p));
        for i = 1:numRods
        ...
        g = GDOT;
    case 'dGdp'  %  Acceleration/dynamics level :constraint forces are necessary so that the acceleration also respects the rigid rod
        numRods = size(omega.R,1);       
        dGdp = zeros(numRods,size(omega.X0,1),class(X.p)); %Lagrange multipliers λ
        for i = 1:numRods
            dGdp(i,:) = X.p'*(hVars.RhatRhat{i});
        end
        g = dGdp;
    case 'dGDOTdp'  %velocity-level constraint change when position changes
        numRods = size(omega.R,1);  
    ...
    case 'dGDOTdRL'
        %todo
    case 'dGdL'  % rod length derivative for rod actuation
        %todo
    case 'dGDOTdL'
        %todo
end
